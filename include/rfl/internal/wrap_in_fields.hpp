...
namespace rfl {
  namespace internal {
    ...
    auto wrap_in_fields(auto&& _tuple, Fields&&... _fields) {
      constexpr auto size = std::tuple_size_v<std::remove_cvref_t<decltype(_tuple)>>;
      ...
      if constexpr (i == size) {
        ...
        if constexpr (is_flatten_field_v<Type>) {
          ...
          constexpr auto n_skip =
            std::tuple_size_v<std::remove_cvref_t<flattened_ptr_tuple_t<typename Type::Type>>>;
          ...
          auto       new_field =
            rfl::make_field<lit_name_v<std::remove_cvref_t<decltype(name_literal)>>>(std::move(value
            ));
