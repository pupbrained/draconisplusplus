...
namespace rfl::parsing {
  ...
  struct Parser<R, W, std::shared_ptr<T>, ProcessorsType> {
    ...
    static Result<std::shared_ptr<T>> read(const R& _r, const InputVarType& _var) noexcept {
      if (_r.is_empty(_var)) {
        return std::shared_ptr<T>();
      }
      const auto to_ptr = [](auto&& _t) { return std::make_shared<T>(std::move(_t)); };
      return Parser<R, W, std::remove_cvref_t<T>, ProcessorsType>::read(_r, _var).transform(to_ptr);
    }

    template <class P>
    static void write(const W& _w, const std::shared_ptr<T>& _s, const P& _parent) noexcept {
      if (!_s) {
        ParentType::add_null(_w, _parent);
        return;
      }
      Parser<R, W, std::remove_cvref_t<T>, ProcessorsType>::write(_w, *_s, _parent);
    }
