...
namespace rfl {
  namespace parsing {
    ...
    concept IsWriter = requires(
      ...
      { w.add_array_to_array(size, &arr) } -> std::same_as<typename W::OutputArrayType>;

      /// Adds an empty object to an existing array. Returns the new
      /// object for further modification.
      { w.add_object_to_array(size, &arr) } -> std::same_as<typename W::OutputObjectType>;
      ...
      /// modification.
      { w.add_array_to_object(name, size, &obj) } -> std::same_as<typename W::OutputArrayType>;
      ...
      /// modification.
      { w.add_object_to_object(name, size, &obj) } -> std::same_as<typename W::OutputObjectType>;

      /// Adds a basic value (bool, numeric, string) to an array. Returns an
      /// OutputVarType containing the new value.
      { w.add_value_to_array(basic_value, &arr) } -> std::same_as<typename W::OutputVarType>;

      /// Adds a basic value (bool, numeric, string) to an existing object. The
      /// key or name of the field is signified by `name`. Returns an
      /// OutputVarType containing the new value.
      { w.add_value_to_object(name, basic_value, &obj) } -> std::same_as<typename W::OutputVarType>;
      ...
      /// OutputVarType containing the null value.
      { w.add_null_to_object(name, &obj) } -> std::same_as<typename W::OutputVarType>;
