...
namespace rfl {
  namespace parsing {
    ...
    struct Parent {
      ...
      static OutputArrayType add_array(const W& _w, const size_t _size, const ParentType& _parent) {
      ...
      static OutputVarType add_null(const W& _w, const ParentType& _parent) {
        ...
        if constexpr (std::is_same<Type, Array>()) {
          ...
          if constexpr (supports_attributes<std::remove_cvref_t<W>>) {
            return _w.add_null_to_object(_parent.name_, _parent.obj_, _parent.is_attribute_);
          } else {
      ...
      static OutputVarType add_value(const W& _w, const T& _var, const ParentType& _parent) {
        using Type = std::remove_cvref_t<ParentType>;
        if constexpr (std::is_same<Type, Array>()) {
          return _w.add_value_to_array(_var, _parent.arr_);
        } else if constexpr (std::is_same<Type, Object>()) {
          if constexpr (supports_attributes<std::remove_cvref_t<W>>) {
            return _w.add_value_to_object(_parent.name_, _var, _parent.obj_, _parent.is_attribute_);
          } else {
