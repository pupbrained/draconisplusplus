   ...
   public:
    ...
    void call_destructors_where_necessary() const {
      if constexpr (_i < size_) {
        ...
        using CurrentType = std::remove_cvref_t<std::tuple_element_t<_i, TupleType>>;
        if constexpr (!std::is_array_v<CurrentType> && std::is_pointer_v<CurrentType> &&
    ...
    void handle_missing_fields_impl(std::optional<Error>* _err) const noexcept {
      if constexpr (_i < size_) {
        if (i_ == _i) {
          ...
          if constexpr (_all_required || is_required<CurrentType, _ignore_empty_containers>()) {
            *_err = Error("Field " + std::to_string(_i) + " was required, but missing.");
            return;
          } else {
      ...
      if constexpr (_i < size_) {
        if (i_ == _i) {
          ...
          if (res) {
            ...
            *_err =
              Error("Failed to parse field " + std::to_string(_i) + ": " + res.error()->what());
          }
          return;
        }
        read_impl<_i + 1>(_var, _err);
      } else {
        *_err = Error("Expected " + std::to_string(size_) + " fields, but got at least one more.");
        return;
      }
    }
