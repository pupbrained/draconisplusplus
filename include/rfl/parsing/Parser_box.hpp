  ...
  namespace parsing {
    ...
    struct Parser<R, W, Box<T>, ProcessorsType> {
      ...
      static Result<Box<T>> read(const R& _r, const InputVarType& _var) noexcept {
        ...
        return Parser<R, W, std::remove_cvref_t<T>, ProcessorsType>::read(_r, _var).transform(to_box
        );
      }

      template <class P>
      static void write(const W& _w, const Box<T>& _box, const P& _parent) noexcept {
        Parser<R, W, std::remove_cvref_t<T>, ProcessorsType>::write(_w, *_box, _parent);
      }
