...
namespace rfl {
  namespace parsing {
    ...
    struct Parser<R, W, std::string_view, ProcessorsType> {
      ...
      static Result<std::string_view> read(const R& _r, const InputVarType& _var) noexcept {
        static_assert(
          always_false_v<R>,
      ...
      template <class P>
      static void write(const W& _w, const std::string_view& _str, const P& _p) noexcept {
        Parser<R, W, std::string_view, ProcessorsType>::write(_w, std::string(_str), _p);
      }
