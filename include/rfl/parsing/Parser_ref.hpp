  ...
  namespace parsing {
    ...
    struct Parser<R, W, Ref<T>, ProcessorsType> {
      ...
      static Result<Ref<T>> read(const R& _r, const InputVarType& _var) noexcept {
        ...
        return Parser<R, W, std::remove_cvref_t<T>, ProcessorsType>::read(_r, _var).transform(to_ref
        );
      }

      template <class P>
      static void write(const W& _w, const Ref<T>& _ref, const P& _parent) noexcept {
        Parser<R, W, std::remove_cvref_t<T>, ProcessorsType>::write(_w, *_ref, _parent);
      }
