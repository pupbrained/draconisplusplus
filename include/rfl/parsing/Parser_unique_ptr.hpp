...
namespace rfl {
  namespace parsing {
    ...
    struct Parser<R, W, std::unique_ptr<T>, ProcessorsType> {
      ...
      static Result<std::unique_ptr<T>> read(const R& _r, const InputVarType& _var) noexcept {
        if (_r.is_empty(_var)) {
          return std::unique_ptr<T>();
        }
        const auto to_ptr = [](auto&& _t) { return std::make_unique<T>(std::move(_t)); };
        return Parser<R, W, std::remove_cvref_t<T>, ProcessorsType>::read(_r, _var).transform(to_ptr
        );
      }

      template <class P>
      static void write(const W& _w, const std::unique_ptr<T>& _s, const P& _parent) noexcept {
        if (!_s) {
