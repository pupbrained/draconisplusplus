# nix/glibc.nix
{ pkgs, self }: # system is already incorporated into pkgs by the time it's passed here
...
let
  ...
  linuxPkgs = pkgs.lib.optionals stdenv.isLinux (with pkgs;
    ...
    ++ (with pkgsStatic; [ # Assuming pkgsStatic is desired
      dbus
      pugixml
      xorg.libxcb
      wayland
  ...
  mkDraconisPackage = { native }: stdenv.mkDerivation {
    name = "draconis++-glibc" + (if native then "-native" else "-generic");
    version = "0.1.0";
    ...
    NIX_ENFORCE_NO_NATIVE = if native then 0 else 1;
